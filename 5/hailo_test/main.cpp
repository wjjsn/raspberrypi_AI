...
void write_all(InputVStream &input, hailo_status &status)
{
    ...
    // std::cout << "Mat size:" << processed.total() * processed.elemSize() << std::endl;
    // std::cout << "Stream size:" << input.get_frame_size() << std::endl;
...
void read_all(OutputVStream &output, hailo_status &status)
{
    ...
    // 第一个 float 表示检测框数量
    int num_detections = static_cast<int>(out[0]);
    ...
    for (int i = 0; i < num_detections; i++) {
        int idx = 1 + i * BOX_SIZE;

        float x1 = out[idx + 0];
        float y1 = out[idx + 1];
        float x2 = out[idx + 2];
        float y2 = out[idx + 3];
        float score = out[idx + 4];
        float cls = out[idx + 5];
