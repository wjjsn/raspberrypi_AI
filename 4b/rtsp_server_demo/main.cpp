...
// ----------- C 风格回调（GStreamer 需要这个） -----------
extern "C" void media_unprepared_cb(GstRTSPMedia* media, gpointer user_data)
{
    (void)media;
...
extern "C" void media_configure_cb(GstRTSPMedia* factory_media,
                                   GstRTSPMedia* media,
                                   gpointer user_data)
{
    (void)factory_media;
    ...
    g_signal_connect(media, "unprepared",
                     (GCallback)media_unprepared_cb, ctx);
    ...
    std::thread(capture_thread, ctx).detach();
}
// -----------------------------------------------------------
